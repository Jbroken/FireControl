...
<body>
	<div class="main_header">
		...
		<div class="header_right">
			<span id="sysTime"></span> <span id="week"></span> <span>${user.relname }，您好！</span>
			<a href="javascript:logout();"><img src="./images/out.gif"
				border="0" /></a>
		</div>
	</div>
	<div class="main_content">
						...
						<ul>
							<c:forEach items="${menu.subMenu}" var="sub">
								<c:if test="${sub.hasMenu}">
									<c:choose>
										<c:when test="${not empty sub.menurel}">
											<li><a href="${sub.menurel }" target="mainFrame">${sub.menuname }</a></li>
										</c:when>
										<c:otherwise>
		...
		<div class="content_right">
			<table cellspacing="0" cellpadding="0">
				<tr
					...
					<td style="height:29px;line-height:29px;"><div
							class="content_title">app下载</div></td>
					<td
				...
				<tr>
					...
					<td style="background-color:#FAFBFD;"><iframe name="mainFrame"
							id="mainFrame" frameborder="0" src="./download/app.html"
			...
			<div class="sys_bottom">Copyright &copy; 2016 泸州公安消防日常检查管理系统</div>
		</div>
	</div>
		...
		function getTime() {
			...
			$("#sysTime").html(
					y + "年" + (m > 9 ? m : ("0" + m)) + "月"
							...
							+ (i > 9 ? i : ("0" + i)) + ":"
							+ (s > 9 ? s : ("0" + s)));
			$("#week").html(dayNames[date.getDay()]);
		}
